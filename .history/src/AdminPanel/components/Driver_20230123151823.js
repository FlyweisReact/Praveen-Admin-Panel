...
const Driver = () => {
  ...
  function AddWallet(props) {
    ...
    const fetchAs = async () => {
      try {
        const { data } = await axios.get(
          `http://ec2-15-206-210-177.ap-south-1.compute.amazonaws.com:3003/api/admin/driver/${id}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
    ...
    const AddMoney = async (e) => {
      ...
      try {
        const data = await axios.put(
          `http://ec2-15-206-210-177.ap-south-1.compute.amazonaws.com:3003/api/admin/drivers/wallet/${id}`,
          {
            balence,
          }
        );
        ...
        <Modal.Body>
          <Container>
            <Form onSubmit={AddMoney}>
              <p style={{border : '1px solid black' , padding : '10px'}}> Amount :   {data?.wallet?.balance ? data?.wallet?.balance : '0'}</p>
  ...
  return (
    <>
      ...
      />
      <AddWallet show={open} onHide={() => setOpen(false)} />
