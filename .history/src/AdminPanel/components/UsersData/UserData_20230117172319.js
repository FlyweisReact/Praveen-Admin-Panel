...
const UserData = () => {
  ...
  const blockHandler = async (id) => {
    try{
        const data = await axios.put(`http://ec2-15-206-210-177.ap-south-1.compute.amazonaws.com:3003/api/admin/users/${id}/block`)
        alert(data.message)
        fetchData()
    }catch(err){
      console.log(err)
            ...
            <tbody>
              {data?.requiredUsers?.map((i) => (
                <tr key={i._id}>
                  ...
                  <td> {i.birthDate} </td>
                  <td> {i.role} </td>
                  <td> {i.isBlocked ? "Yes" : <Button onClick={(e) => blockHandler()}>Block</Button>} </td>
                  <td>
                    <Button
